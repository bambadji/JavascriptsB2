...
export class ListNiveauComponent implements OnInit {
  ...
  ngOnInit(): void {
			...
			(erreur)=>{
				...
				switch (erreur.status) {
		          ...
		          case 403:
		            ...
		            break;
		          default:
		            this.openSnackBar("Erreur !!! : impossible de recuperer les niveaus. Veiller reessayer","X",15000);
		            break;
		        }
			}
		);
	...
	goToPageEdit(id:number){
		this.router.navigate(["edit-niveau",id]);
	}
	...
	onDelete(id:number){
		let title = "ATTENTION !!!";
		let message = "Voulez vous vraiment supprimer ce Niveau"
	        	...
	        	if (data == true) {
	        		...
	        		this.niveauService.deleteNiveau(id).subscribe(
						(data)=>{
							...
							this.doGetAll();
							this.openSnackBar("Succés : Niveau supprimer","OK",3000);
						},
						(erreur)=>{
							this.suppression =false;
							console.log("erreur : "+JSON.stringify(erreur));
							switch (erreur.status) {
					          ...
					          case 500:
					            ...
					            break;
					          default:
					            this.openSnackBar("Erreur !!! : impossible de supprimer cet niveau. Veiller reessayer","X",15000);
					            break;
		        			}
						}
					);
