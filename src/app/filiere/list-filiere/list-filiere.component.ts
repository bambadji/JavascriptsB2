...
export class ListFiliereComponent implements OnInit {
  ...
  chargement:boolean=true;// active progreess spinner for loading list
	...
	displayedColumns = ['id', 'libelle', 'description', 'duree', 'action'];
			...
			(erreur)=>{
				...
				switch (erreur.status) {
		          ...
		          case 403:
		            ...
		            break;
		          default:
		            this.openSnackBar("Erreur !!! : impossible de recuperer les filieres. Veiller reessayer","X",15000);
		            break;
		        }
			}
		);
	...
	goToPageEdit(id:number){
		this.router.navigate(["edit-filiere",id]);
	}
	...
	onDelete(id:number){
		let title = "ATTENTION !!!";
		let message = "Voulez vous vraiment supprimer ce Filiere"
	        	...
	        	if (data == true) {
	        		...
	        		this.filiereService.deleteFiliere(id).subscribe(
						(data)=>{
							...
							this.doGetAll();
							this.openSnackBar("Succés : Filiere supprimer","OK",3000);
						},
						(erreur)=>{
							this.suppression =false;
							console.log("erreur : "+JSON.stringify(erreur));
							switch (erreur.status) {
					          ...
					          case 500:
					            ...
					            break;
					          default:
					            this.openSnackBar("Erreur !!! : impossible de supprimer cet filiere. Veiller reessayer","X",15000);
					            break;
		        			}
						}
					);
